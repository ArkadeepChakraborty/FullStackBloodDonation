...
adminRouter.get('/getAllRequest', async (req, res) => {
    try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                let donor_details = await Doner.findById(did);

                const requestObject = data[i].toObject();
    
                if (user_details) {
                    const userObject = user_details.toObject(); // Convert Mongoose document to plain object
                    requestObject.uname= userObject.name
                }
                if(donor_details){
                    donor_details = donor_details.toObject(); // Convert Mongoose document to plain JavaScript object
                    requestObject.dname= donor_details.name
                }

                temp.push(requestObject)
            }
